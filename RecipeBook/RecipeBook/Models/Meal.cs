...
public class Meal
{
    ...
    public string BuildIngredientsText()
    {
        var lines = new List<string>();
        for (var i = 1; i <= 20; i++)
        {
            var ingredient = GetType().GetProperty($"StrIngredient{i}")?.GetValue(this)?.ToString()?.Trim();
            var measure = GetType().GetProperty($"StrMeasure{i}")?.GetValue(this)?.ToString()?.Trim();
            ...
            lines.Add(string.IsNullOrWhiteSpace(measure)
                ? $"- {ingredient}"
                : $"- {ingredient} ({measure})");
        }
